...
const AllMovies = () => {
  ...
  const handleSearchChange = (e) => {
    ...
    const filteredMovies = data?.map((m) =>{
      ...
      return{...m,score}
  }).filter((m)=>m.score>=62).sort((a,b)=>b.score-a.score)

    dispatch(setFilteredMovies(...setFilteredMovies,filteredMovies));
  };

  const handleGenreClick = (genreId) => {
    dispatch(setMoviesFilter({ selectedGenre: genreId}));
    const filterByGenre = data.filter((movie) => movie.genre === genreId);
    dispatch(setFilteredMovies(...setFilteredMovies,filterByGenre));
  };

  const handleYearChange = (year) => {
    dispatch(setMoviesFilter({  selectedYear: year}));
    const filterByYear = data.filter((movie) => movie.year === +year);
    dispatch(setFilteredMovies(...setFilteredMovies,filterByYear));
  };
